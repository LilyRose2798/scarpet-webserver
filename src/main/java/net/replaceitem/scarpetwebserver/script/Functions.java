...
public class Functions {
    ...
    @ScarpetFunction
    public void ws_add_sse_route(Context context, Webserver webserver, String path, Value callback) {
        webserver.addSSERoute(path, context.host, callback);
    }

    @ScarpetFunction
    public boolean sse_add_header(SSEConnection connection, String header, String value) {
        return connection.addHeader(header, value);
    }

    @ScarpetFunction
    public boolean sse_override_response(Context context, SSEConnection connection, Value callback) {
        return connection.overrideResponse(context.host, callback);
    }

    @ScarpetFunction(maxParams = 4)
    public boolean sse_send_message(Context context, SSEConnection connection, Optional<String> event, String data, Optional<Value> callback) {
        return connection.sendMessage(event, data, context.host, callback);
    }

    @ScarpetFunction(maxParams = 2)
    public boolean sse_close(Context context, SSEConnection connection, Optional<Value> callback) {
        return connection.close(context.host, callback);
    }

    @ScarpetFunction
    public boolean sse_is_closed(SSEConnection connection) {
